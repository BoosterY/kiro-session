...
**方案决策**：PTY 注入。
1. 生成 temp JSON（完整 ConversationState）
2. `pty.fork()` 在子进程里 `cd <dir> && kiro-cli chat`
3. 监听 PTY 输出，检测到 kiro-cli prompt（`> `）后注入 `/chat load <path>`
4. 桥接 PTY 到用户终端，用户直接继续对话
5. 无 TTY 环境（skill 调用等）自动 fallback 到打印指令

已否决的方案：
- `--resume`：限当前目录，只恢复最近一个
- 复制 session 到目标目录：v1 需要写 DB，违反 read-only
- stdin pipe：kiro-cli 处理完 EOF 就退出，无法继续交互
- `/context add`：只加 context 不加 history，不是真正恢复
...
**当前状态**：✅ 已实现
- `kiro-session search <query>` — FTS5 全文搜索，零 token
- 过滤器：`--dir`, `--recent`, `--file`, `--cmd`
- picker 内：`/` 本地过滤（名称、tags、目录），`s` FTS 全文搜索（替换列表为搜索结果）
...
- detail page 操作：`[r]` resume、`[1-N]` topic resume、`[n]` rename、`[t]` tags、`[v]` save、`[d]` delete、`[i]` index、`[f]` feedback
...
**待优化**：
- 垃圾 session（headless LLM 调用产生的）可靠自动清理
...
- 垃圾 session 可靠清理（headless LLM 调用产生的）
- 中文分词精度
- LLM 搜索优化（避免垃圾 session）
